...
const systems = [
  {
    ...
    image: "https://res.cloudinary.com/dzyxm0rhg/image/upload/f_auto,q_auto/v1787153413/autoshow_zbj29b.png",
    cure:
  ...
  },
  {
    ...
    image: "https://res.cloudinary.com/dzyxm0rhg/image/upload/f_auto,q_auto/v1787153413/callboda_vnlf0k.png",
    cure:
  ...
  },
  {
    ...
    image: "https://res.cloudinary.com/dzyxm0rhg/image/upload/f_auto,q_auto/v1787153413/sidelined-gem_spfnoq.png",
    cure:
      ...
      <div className="border-b border-ink/10 px-6 py-16 md:px-16 md:py-24">
        <Reveal>
          ...
          <p className="mt-6 max-w-xl leading-[1.8] text-muted">
            ...
            ways of working. Hover to look. Tap to use.
          </p>
        </Reveal>
      </div>
        ...
        {systems.map((s) => (
          <Reveal key={s.number}>
            <a
              ...
              className="group relative block overflow-hidden border-b border-ink/10 py-10 last:border-b-0 transition-colors md:py-14"
            >
              {/* The reveal — image bleeds in on hover (desktop), stays hidden on mobile */}
              <div
                aria-hidden="true"
              ...
              >
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  className="h-full w-full scale-105 object-cover transition-transform duration-[1500ms] ease-out group-hover:scale-100"
                />
                {/* Directional gradient: dark left (for text), fading right (for image) */}
                <div className="absolute inset-0 bg-gradient-to-r from-deep via-deep/90 to-deep/20" />
              </div>

              {/* Content — always readable, sits above the reveal */}
              <div className="relative z-10">
                ...
                {/* Expanded section — cure + mobile image */}
                <div className="grid grid-rows-[0fr] transition-[grid-template-rows] duration-500 ease-out group-hover:grid-rows-[1fr] group-focus:grid-rows-[1fr]">
                  <div className="overflow-hidden">
                    <div className="flex flex-col gap-6 pt-6 md:flex-row md:items-end md:justify-between md:pl-16">
                      {/* Mobile: image shown inline inside the expanded section */}
                      <div className="md:hidden">
                        <img
                          src={s.image}
                          alt={s.title}
                          loading="lazy"
                          className="h-56 w-full border border-ink/15 object-cover shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
                        />
                        <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.25em] text-muted">
                          {s.liveLabel} — live system
                        </p>
                      </div>
